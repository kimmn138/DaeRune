...
1. **CCTV 화면 수집을 컴포넌트 태그 방식으로** — 컴포넌트 참조 배열(`TArray<UStaticMeshComponent*>`)은 BP 디테일 패널에서 배선할 수 없다. BP에서 화면 메시에 `CctvScreen` 태그를 달면 `BeginPlay`에서 이름 순으로 수집한다.
...
2. BP 생성: `BP_S2Lever` / `BP_S2SafeButton` / `BP_S2PuzzleTerminal`(각 프롭, 메시 + "F" 위젯) · `BP_S2CodeScreen`(숫자 아틀라스 머티리얼) · `BP_S2SwitchPuzzle` · `BP_S2CctvBoard`(화면 6개 + `CctvScreen` 태그 + 텍스처 파라미터 `ScreenTex`) · `BP_S2Safe`(`PartClass = BP_S2Part`) · `BP_S2SlidePuzzle`
...
#### 16.8.2 `BP_S2CodeScreen` (부모 `ADRS2CodeScreen`)

숫자 표시판. 8퍼즐/스위치의 비밀번호 자리 표시와 금고 입력 표시에 **같은 BP를 쓴다**(3개 배치).

| 항목 | 내용 |
|---|---|
| `ScreenMesh` | 평면 메시 + 숫자 표시용 머티리얼 |
| **`OnDigitsChanged(TArray<int32>)`** ★ | **필수 구현.** 배열을 받아 화면에 그린다. **음수(−1)는 "미공개"** 이므로 `-` 또는 빈칸으로 표시한다 |

**구현 방법 2가지**
- **머티리얼 파라미터**: 숫자 0~9 아틀라스 텍스처 + `Scalar` 파라미터로 UV 오프셋 이동. 자리 수만큼 머티리얼 슬롯 분리.
- **3D 텍스트**: `TextRender` 컴포넌트를 자리 수만큼 붙이고 `SetText`. 구현이 간단해 먼저 이 방식으로 검증하는 것을 권장.
...
#### 16.8.4 `BP_S2SwitchPuzzle` (부모 `ADRS2SwitchPuzzle`) — 스위치 퍼즐

| 프로퍼티 | 값 |
|---|---|
| `LeverCount` | `5` (기본값) |
| `BulbCount` | `9` (기본값) |
| `RequiredRounds` | `3` (기본값) |
| `MinBitsPerLever` / `MaxBitsPerLever` | `3` / `5` (기본값) — 난이도 조절 |
| `CodeScreen` | `BP_S2CodeScreen` 인스턴스 |

**컴포넌트 구성** — 전구 9개와 성공 표시 3개는 **BP에서 메시 컴포넌트로 추가**한다.

| 연출 훅 | 필수 | 내용 |
|---|---|---|
| **`OnBulbsChanged(int32 NewBulbBits)`** | ★필수 | 비트마스크. **비트 i = 전구 i**. `NewBulbBits & (1 << i)`로 켜짐 판정 → 전구 머티리얼/라이트 토글. **이걸 비우면 퍼즐을 풀 수 없다** |
| `OnLeversChanged(int32 NewLeverBits)` | 권장 | 레버 ON/OFF 자세 |
| **`OnRoundsClearedChanged(int32 N)`** | ★필수 | 성공 표시 N개 점등 (n/3) |
| `OnPuzzleSolvedVisual(int32 Digit)` | 권장 | 3라운드 완료 연출 |

> 전구 9개를 배열로 관리하려면 BP에서 `Array of Static Mesh Component` 변수를 만들고 `Construction Script` 또는 `BeginPlay`에서 채운 뒤 `OnBulbsChanged`에서 인덱스로 접근하는 방식이 편하다.

#### 16.8.5 `BP_S2CctvBoard` (부모 `ADRS2CctvBoard`) — CCTV

| 프로퍼티 | 값 |
|---|---|
| `ScreenComponentTag` | `CctvScreen` (기본값) |
...
**컴포넌트 구성** ★가장 실수하기 쉬운 부분

1. 화면 메시 **6개**를 `StaticMeshComponent`로 추가한다.
2. **각 컴포넌트의 `Component Tags`에 `CctvScreen`을 추가한다.** ← 이게 없으면 `[S2CCTV] 화면 메시를 찾지 못했습니다` Error가 뜨고 CCTV가 동작하지 않는다.
3. **컴포넌트 이름 순서가 화면 인덱스**가 된다. `Screen_0` ~ `Screen_5`처럼 정렬 가능한 이름을 쓴다.
4. 화면 머티리얼에 **`ScreenTex`라는 이름의 Texture Parameter**를 만든다. 이름이 다르면 텍스처가 바뀌지 않고 초기 상태로 멈춘다.

**머티리얼 요구사항**: `Texture Sample`의 텍스처를 `TextureSampleParameter2D`로 만들고 파라미터 이름을 `ScreenTex`로 지정. Emissive 출력으로 연결하면 화면처럼 보인다.

> 기존 아트 에셋에 CCTV 메시가 있다(`Content/DaeRuneAssets/Map/Stage2_v1/cctv1~6`, `cctv11~61`). 재활용 가능하다.

#### 16.8.6 `BP_S2Safe` (부모 `ADRS2Safe`) — 금고

| 프로퍼티 | 값 |
|---|---|
| `InputDisplay` | `BP_S2CodeScreen` 인스턴스 (입력 진행 표시) |
...
- `BP_S2Safe` 1 + `BP_S2SafeButton` **10** (`PropIndex` 0~9) + 초기화 버튼 1 + `BP_S2CodeScreen` 1
...
| 퍼즐/금고 | `CodeScreen` / `InputDisplay` | 각 `BP_S2CodeScreen` |
...
| CCTV 화면 컴포넌트 | **`CctvScreen`** 컴포넌트 태그 |
...
| CCTV 화면이 안 바뀐다 | `CctvScreen` 태그 누락 또는 머티리얼 파라미터 이름 불일치 | §16.8.5 |
